...
import {Circle, CircleProps, Layout, Line, Node, Rect, RectProps, Txt} from "@motion-canvas/2d/lib/components";
...
import {ColorPalette} from "./pipeline";
...
export default makeScene2D(function* (view) {
    ...
    const fullComponents = createRef<Node>()
        ...
        <Node ref={fullComponents}>
            ...
            <Rect ref={runner2} x={boxWidth * spacingFactor} {...headerRectStyle}>
                ...
                <Rect ref={runner2Content} y={boxHeight} height={boxHeight} {...contentRectStyle}>
                    <Layout direction={'column'} width={() => runner2().width()} gap={30} padding={[40, 20, 0, 20]}
                            layout>
                        <Txt ref={job2Text}/>
            ...
            {/* RUNNER 2 EXPANDED CONTENT */}
            <Rect ref={runner2ContentExpandRect} opacity={0} x={boxWidth * spacingFactor} y={boxHeight}
                  height={boxHeight} {...contentRectStyle}>
    ...
    yield* beginSlide("Add Braces")

    const jobBrace = createRef<Brace>()
    const jobText = createRef<Txt>()

    const actionBox = createRef<SurroundingRectangle>()
    const actionText = createRef<Txt>()

    const stepBrace = createRef<Brace>()
    ...
    view.add(
        <>
            <Brace
                ...
                sharpness={1.4}
                lineWidth={17}
                nodes={[runner1Content(), runner1Step4Text()]} buffer={40}
                start={0.5} end={0.5}
            />
            <Txt ref={jobText} text={""} fontSize={70} fontFamily={"Fira Code"} fill={ColorPalette.pink}
                 x={-400} y={-15}/>

            <SurroundingRectangle ref={actionBox} opacity={0} stroke={ColorPalette.yellow} radius={40} buffer={150}
                                  lineWidth={10} nodes={event()}/>
            <Txt ref={actionText} text={""} fontSize={120} fontFamily={"Fira Code"} fill={ColorPalette.yellow}
                 x={() => view.width() / -2 + 350} y={() => view.height() / 2 - 80}/>
            ...
            <Brace
                ...
                sharpness={0.7}
                lineWidth={10}
                nodes={[runner2Step6(), runner2Step5()]} buffer={140}
                start={0.5} end={0.5}
            />
            <Txt ref={stepText} text={""} fontSize={70} fontFamily={"Fira Code"} fill={ColorPalette.blue}
                 x={170} y={240}/>
        </>
    )
    ...
    yield* chain(
        ...
        sequence(
            0.3,
            all(
                fullComponents().scale(0.8, 1),
                fullComponents().position.y(-100, 1),
            ),
            chain(
                all(
                    actionBox().opacity(1, 1),
                    actionBox().nodes([event(), runner2Step6()], 1)
                ),
                actionText().text("Action", 1)
            ),
        ),
        // JOB
        sequence(
